		...
		return (
			...
			>
				...
				<div className="columns is-mobile is-gapless is-multiline summary-block">
					...
					<div className="column is-6">{text.taxtIncluded}</div>
					<div className="column is-6 has-text-right price">
						{helper.formatCurrency(cart.tax_included_total, settings)}
					</div>
					<div className="column is-12">
						<hr className="separator" />
					</div>
					<div className="column is-6 total-text">{text.orderTotal}</div>
					<div className="column is-6 total-price">
						{helper.formatCurrency(cart.order_total, settings)}
					</div>
				</div>
			</div>
		);
