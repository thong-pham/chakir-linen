...
export default class ProductDetails extends React.Component {
  ...
  changeImagePosition(colorName){
      const { images } = this.state;
      let option = null;
      images.forEach((image, index, object) => {
          if (image.alt.toLowerCase() === colorName){
              option = image;
              object.splice(index,1);
          }
      });
      images.unshift(option);
  }

  onOptionChange(optionName, optionId, valueId) {
    ...
    if (optionName === 'Color'){
        const { product } = this.props;
        const option = product.options.filter(option => option.name === 'Color')[0];
        option.values.forEach(value => {
            if (value.id === valueId){
                this.changeImagePosition(value.name.toLowerCase());
            }
        });
    }
  }
