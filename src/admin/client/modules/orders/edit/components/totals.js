...
const OrderTotals = ({order, settings}) => {
  ...
  const orderTotal = helper.formatCurrency(order.order_total, settings);
  ...
  return (
    <div>
      ...
      <div className={style.total + " row " + style.orderTotal}>
        <div className="col-xs-7">Order Total</div>
        <div className="col-xs-5">{orderTotal}</div>
      </div>
      ...
      <div className={style.total + " row " + style.orderTotal}>
        <div className="col-xs-7">{messages.amountPaid}</div>
