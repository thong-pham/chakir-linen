...
export const confirmEmail = (data, history) => (dispatch, getState) => {
    return axios.post(URL + "users/confirmEmail", data)
            .then(function (response) {
...
export const login = (user, history) => (dispatch, getState) => {
    ...
    return axios.post(URL + "users/login", user)
            ...
            .catch(function (error) {
                const response = error.response;
                dispatch({ type: t.USER_LOGIN_REJECTED, payload: response.data });
...
export const fetchUserData = (userId) => (dispatch, getState) => {
    ...
    return axios.get(URL + "users/" + userId)
            ...
            .then(function (data) {
                //console.log(data);
                dispatch({ type: t.FILL_USER, payload: data });
                return data;
            })
            .catch(function (error) {
                const response = error.response;
...
export const saveUserData = (data) => (dispatch, getState) => {
    ...
    return axios.put(URL + "users/" + userId, data)
            ...
            .then(function (data) {
                //console.log(data);
                dispatch({ type: t.USER_UPDATE_FULFILLED, payload: data });
                return data;
            })
            .catch(function (error) {
                const response = error.response;
                dispatch({ type: t.USER_UPDATE_REJECTED, payload: response.data });
                //console.log(response);
                throw response;
          })
}
...
export const changePassword = (data) => (dispatch, getState) => {
    const {app} = getState();
    const userId = app.user._id;
    return axios.put(URL + "users/" + userId + "/changePassword", data)
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
...
export const fetchOrders = (id) => (dispatch, getState) => {
    //dispatch({type: t.USER_LOGIN_STARTED});
    return axios.get(URL + "orders?paid=true&user_id=" + id)
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
                //console.log(data);
                dispatch({ type: t.FILL_ORDERS, data });
                return data;
            })
            .catch(function (error) {
                const response = error.response;
...
export const fetchShippingAddress = (userId) => (dispatch, getState) => {
    //dispatch({type: t.USER_LOGIN_STARTED});
    return axios.get(URL + "users/" + userId + "/shipping_address")
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
                //console.log(data);
                dispatch({ type: t.FILL_ADDRESSES, data });
                return data;
            })
            .catch(function (error) {
                const response = error.response;
...
export const addShippingAddress = (address, history) => (dispatch, getState) => {
    const {app} = getState();
    //address.id = app.shippingAddresses.length + 1;
    const userId = app.user._id;

    return axios.post(URL + "users/" + userId + "/shipping_address", address)
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
...
export const updateShippingAddress = (address, history) => (dispatch, getState) => {
    ...
    return axios.put(URL + "users/" + userId + "/shipping_address/" + addressId, address)
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
...
export const deleteShippingAddress = (addressId, history) => (dispatch, getState) => {
    const {app} = getState();
    const userId = app.user._id;
    return axios.delete(URL + "users/" + userId + "/shipping_address/" + addressId)
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
                dispatch({ type: t.DELETE_ADDRESS, addressId });
                return data;
            })
            .catch(function (error) {
                const response = error.response;
...
export const setDefaultShipping = (addressId, history) => (dispatch, getState) => {
    const {app} = getState();
    const userId = app.user._id;
    return axios.post(URL + "users/" + userId + "/shipping_address/" + addressId + "/default_shipping")
            .then(function (response) {
                return response.data;
            })
            .then(function (data) {
...
export const editShippingAddress = (id, history) => (dispatch, getState) => {
    dispatch({type: t.EDIT_ADDRESS, id});
}

export const searchOrder = (words, history) =>  (dispatch, getState) => {
...
export const addCartItem = item => (dispatch, getState) => {
  ...
  return api.ajax.cart.addItem({item,user}).then(({status, json}) => {
    dispatch(receiveCart(json))
...
export const updateCartShippingMethod = (method_id, shipping_price) => (dispatch, getState) => {
  api.ajax.cart.update({payment_method_id: null, shipping_method_id: method_id, shipping_price: shipping_price}).then(({status, json}) => {
    dispatch(receiveCart(json))
    dispatch(fetchPaymentMethods())
  }).catch(error => {});
}
