...
class OrdersService {
  ...
  getFilter(params = {}) {
    ...
    const user_id = parse.getObjectIDIfValid(params.user_id);
  ...
  getValidDocumentForInsert(data) {
    return mongo.db.collection('orders').find({}, {number: 1}).sort({number: -1}).limit(1).toArray().then(items => {
      ...
      order.firstName = parse.getString(data.firstName);
      order.lastName = parse.getString(data.lastName);
      ...
      order.user_id = parse.getObjectIDIfValid(data.user_id);
  ...
  changeProperties(order, orderStatuses, shippingMethods, paymentMethods) {
    if (order) {
      ...
      let grand_total = Number((sum_items_price_total).toFixed(Math.max(0, ~~ 2))) + shipping_total - discount_total;
      if (order.shipping_address.state.toLowerCase() === 'ca' || order.shipping_address.state.toLowerCase() === 'california'){
          tax_included_total = Number(((grand_total * 7.75)/100).toFixed(Math.max(0, ~~ 2)));
      }
      else {
          tax_included_total = 0;
      }
      order_total = Number((grand_total + tax_included_total).toFixed(Math.max(0, ~~ 2)));
      order.weight_total = sum_items_weight;
      order.discount_total = discount_total; //sum(items.discount_total)+sum(discounts.amount)
      order.subtotal = Number((sum_items_price_total).toFixed(Math.max(0, ~~ 2))); //sum(items.price_total)
  ...
  updateCustomerStatistics(customerId) {
    if(customerId){
      return this.getOrders({ user_id: customerId }).then(orders => {
        let totalSpent = 0;
        let ordersCount = 0;
  ...
  updateUserStatistics(userId) {
    if(userId){
      return this.getOrders({ user_id: userId }).then(orders => {
        let totalSpent = 0;
        let ordersCount = 0;
        ...
        return UsersService.updateUserStatistics(userId, totalSpent, ordersCount);
      })
    } else {
      return null;
    }
